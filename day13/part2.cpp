...
pair<long long, long long> solve(long long a, long long b, long long c,
  ...
  if ((e * d - b * f) % det == 0 && (a * f - e * c) % det == 0) {
    long long x = (e * d - b * f) / det;
    long long y = (a * f - e * c) / det;
    return {x, y};
  } else {
    return {-1, -1};
  }
...
int main() {
  ...
  smatch match;
  vector<pair<long long, long long>> configs;
  if (file.is_open()) {
    string line;
    while (getline(file, line)) {
      ...
      if (regex_search(line, match, re)) {
        if (match.size() == 3) {
          configs.push_back(pair(stoll(match[1].str()), stoll(match[2].str())));
        }
      }
    ...
    for (size_t i = 0; i < configs.size(); i += 3) {
      ...
      auto [x, y] =
          solve(configs[i].first, configs[i + 1].first, configs[i].second,
                configs[i + 1].second, configs[i + 2].first + 10000000000000,
                configs[i + 2].second + 10000000000000);
